...
CREATE TABLE ARTICULOS(
    ...
    CONSTRAINT PK_ART PRIMARY KEY (ARTICULO, COD_FABRICANTE, PESO, CATEGORIA), -- ESTABLECE LA PRIMARY KEY
    CONSTRAINT FK_ART FOREIGN KEY (COD_FABRICANTE) REFERENCES FABRICANTES ON DELETE CASCADE, -- ESTABLECE LA FOREIGN KEY
    CONSTRAINT MAYOR CHECK (PRECIO_VENTA > 0 AND PRECIO_COSTO > 0 AND PESO > 0), -- ESTABLECE QUE PRECIO_VENTA, PRECIO_COSTO Y PESO SEAN MAYOR A 0
    CONSTRAINT CATEGORIA_CT CHECK (CATEGORIA IN ('Primera', 'Segunda', 'Tercera')) -- ESTABLECE LA CATEGORIA EN PRIMERA, SEGUNDA O TERCERA
);
...
CREATE TABLE VENTAS(
    ...
    CONSTRAINT PK_VENTAS PRIMARY KEY(NIF, ARTICULO, COD_FABRICANTE, PESO, CATEGORIA, FECHA_VENTA), -- ESTABLECE LA PRIMARY KEY
    CONSTRAINT FK_VENTAS FOREIGN KEY(NIF) REFERENCES TIENDAS ON DELETE CASCADE, -- ESTABLECE LA FOREIGN KEY
    CONSTRAINT FK2_VENTAS FOREIGN KEY (ARTICULO, COD_FABRICANTE, PESO, CATEGORIA) REFERENCES ARTICULOS ON DELETE CASCADE, -- ESTABLECE LA FOREIGN KEY
    CONSTRAINT MAYOR_VENTAS CHECK( UNIDADES_VENDIDAS > 0) -- ESTABLECE QUE LAS UNIDADES VENDIDAS SEAN MAYOR QUE 0
);
...
CREATE TABLE PEDIDOS(
    ...
    CONSTRAINT PK_PEDIDOS PRIMARY KEY(NIF, ARTICULO,COD_FABRICANTE,PESO,CATEGORIA, FECHA_PEDIDO), -- ESTABLECE LA PRIMARY KEY
    CONSTRAINT FK_PEDIDOS FOREIGN KEY(NIF) REFERENCES TIENDAS ON DELETE CASCADE, -- ESTABLECE LA FOREIGN KEY
    CONSTRAINT FK2_PEDIDOS FOREIGN KEY(ARTICULO, COD_FABRICANTE, PESO, CATEGORIA) REFERENCES ARTICULOS ON DELETE CASCADE, -- ESTABLECE LA FOREIGN KEY
    CONSTRAINT UP_PEDIDOS CHECK(UNIDADES_PEDIDAS > 0), -- ESTABLECE QUE LAS UNIDADES PEDIDAS SEAN MAYOR QUE 0
    CONSTRAINT CATEGORIA_PEDIDOS CHECK(CATEGORIA IN('Primera','Segunda','Tercera')) -- ESTABLECE LA CATEGORIA EN PRIMERA, SEGUNDA O TERCERA
);
